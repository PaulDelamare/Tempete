...
import { buildColumnsFromConfig, ColumnConfig } from "../_components/table/columns";
...
export default function EventPage() {
     ...
     const eventConfigs: ColumnConfig<EventWithRelations>[] = [
          ...
          { key: "datestart", title: "Début", format: (val) => new Date(val as string).toLocaleString() },
          { key: "dateend", title: "Fin", format: (val) => new Date(val as string).toLocaleString() },
          {
               key: "areaId",
               title: "Zone",
          ...
          },
          {
               key: "tagsJoin",
               title: "Tags",
               format: (_, row) => row.tagsJoin.map((t) => t.tag.name).join(", "),
          },
          {
               id: "actions",
               title: "Actions",
                                   ...
                                   <AlertDialogContent>
                                        <AlertDialogHeader>
                                             ...
                                             <AlertDialogDescription>
                                                  Cette action est irréversible. L’événement <strong>{event.name}</strong> sera supprimé définitivement.
                                             </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
     ...
     const eventColumns = buildColumnsFromConfig<EventWithRelations>(eventConfigs);
                    ...
                    <CardHeader>
                         <div className="flex items-start justify-between gap-4">
                              <div>
                                   ...
                                   <CardDescription>Retrouvez ici tous les événements enregistrés.</CardDescription>
                              </div>
                              <CardAction>
               ...
               >
                    {selectedEvent && (
                         <EventForm
                              event={{
                                   ...selectedEvent,
                                   tagsJoin: selectedEvent.tagsJoin?.map((t) => ({ tag: t.tag })) ?? [],
                                   artists: selectedEvent.artists?.map((a) => a.artist) ?? [],
